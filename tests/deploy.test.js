...
test('pin login page and auth endpoint use the household PIN', async () => {
  await resetForTests();
  const previousPin = process.env.HOUSEHOLD_PIN;
  const previousPassword = process.env.HOUSEHOLD_PASSWORD;
  const previousSecret = process.env.AUTH_SECRET;

  process.env.HOUSEHOLD_PIN = '3576';
  delete process.env.HOUSEHOLD_PASSWORD;
  ...
  try {
    ...
    res = await fetch(`${base}/login?next=%2F%2Fevil.example`);
    assert.equal(res.status, 200);
    assert.match(await res.text(), /const next = rawNext\.startsWith\('\/'\) && !rawNext\.startsWith\('\/\/'\) \? rawNext : '\/home'/);
    ...
    assert.equal(res.status, 200);
    assert.match(res.headers.get('set-cookie'), /todo_session=/);
  } finally {
    if (previousPin === undefined) delete process.env.HOUSEHOLD_PIN; else process.env.HOUSEHOLD_PIN = previousPin;
...
test('documents api exposes fake household docs only to authenticated users', async () => {
  await resetForTests();
  const previousPassword = process.env.HOUSEHOLD_PASSWORD;
  const previousSecret = process.env.AUTH_SECRET;
  ...
  try {
    ...
    res = await fetch(`${base}/api/documents`, { headers: { cookie } });
    assert.equal(res.status, 200);
    const body = await res.json();
    assert.ok(body.documents.length >= 4);
    assert.ok(body.documents.some(doc => doc.category === 'Insurance Cards'));
    assert.ok(body.documents.every(doc => doc.source === 'placeholder'));
  } finally {
    if (previousPassword === undefined) delete process.env.HOUSEHOLD_PASSWORD; else process.env.HOUSEHOLD_PASSWORD = previousPassword;
    if (previousSecret === undefined) delete process.env.AUTH_SECRET; else process.env.AUTH_SECRET = previousSecret;
    server.close();
...
test('calendar api exposes Family calendar events only to authenticated household users', async () => {
  await resetForTests();
  const previousPassword = process.env.HOUSEHOLD_PASSWORD;
  const previousSecret = process.env.AUTH_SECRET;
  const previousIcalUrl = process.env.GOOGLE_CALENDAR_ICAL_URL;
  ...
  try {
    let res = await fetch(`${base}/api/calendar`);
    assert.equal(res.status, 401);
    ...
    res = await fetch(`${base}/api/calendar`, { headers: { cookie } });
    assert.equal(res.status, 200);
    const body = await res.json();
    assert.equal(body.events.length, 1);
    assert.equal(body.events[0].id, 'hub-calendar-1');
    assert.equal(body.events[0].summary, 'School concert, gym');
    assert.equal(body.events[0].time, 'All day');
  } finally {
    if (previousPassword === undefined) delete process.env.HOUSEHOLD_PASSWORD; else process.env.HOUSEHOLD_PASSWORD = previousPassword;
    if (previousSecret === undefined) delete process.env.AUTH_SECRET; else process.env.AUTH_SECRET = previousSecret;
    if (previousIcalUrl === undefined) delete process.env.GOOGLE_CALENDAR_ICAL_URL; else process.env.GOOGLE_CALENDAR_ICAL_URL = previousIcalUrl;
...
test('household auth protects app pages and api when enabled', async () => {
  ...
  try {
    ...
    res = await fetch(`${base}/home`, { headers: { cookie }, redirect: 'manual' });
    assert.equal(res.status, 200);

    res = await fetch(`${base}/calendar`, { headers: { cookie }, redirect: 'manual' });
    assert.equal(res.status, 200);

    res = await fetch(`${base}/documents`, { headers: { cookie }, redirect: 'manual' });
    assert.equal(res.status, 200);
  } finally {
    if (previousPassword === undefined) delete process.env.HOUSEHOLD_PASSWORD; else process.env.HOUSEHOLD_PASSWORD = previousPassword;
    if (previousSecret === undefined) delete process.env.AUTH_SECRET; else process.env.AUTH_SECRET = previousSecret;
